...
# ----------------------
# BasicBlock (user's version adapted)
# ----------------------
class BasicBlock(nn.Module):
    def __init__(self, c_in, c_out, num_feature_map=64):
        super(BasicBlock, self).__init__()
        # use padding=1 for kernel_size=3 to keep same spatial dims
        self.conv1 = nn.Conv2d(c_in, c_out, KERNEL_SIZE, STRIDE, padding=1)
        self.conv1_bn = nn.BatchNorm2d(c_out)
        self.conv11 = nn.Conv2d(c_out, num_feature_map, KERNEL_SIZE, STRIDE, padding=1)
        self.conv11_bn = nn.BatchNorm2d(num_feature_map)
        self.conv12 = nn.Conv2d(num_feature_map, c_out, KERNEL_SIZE, STRIDE, padding=1)
...
# ----------------------
# VAEClassifier (adapted input channel -> 3)
# ----------------------
class VAEClassifier(nn.Module):
    def __init__(self, num_feature_map=64, encoder_layer=3, decoder_layers=3, input_channels=3):
        super(VAEClassifier, self).__init__()

        self.encoder_module = nn.ModuleList([BasicBlock(input_channels, num_feature_map)])
        ...
        self.output_layer = nn.ModuleList([BasicBlock(num_feature_map, LAST_DIM), BasicBlock(LAST_DIM, input_channels)])
    ...
    def forward(self, x, deterministic=True, classification_only=True):
        ...
        # trim for 28x28 if needed — but SVHN is 32x32, so keep full
        # original code did x = x[:,:,2:30,2:30] for 28x28 cropping; for SVHN we keep 32x32
        x = torch.sigmoid(x)
...
# ----------------------
# StAEClassifier (adapted input channel -> 3)
# ----------------------
...
# ----------------------
# ResNetEnc (keeps 3-channel handling)
# ----------------------
class ResNetEnc(nn.Module):
    def __init__(self, image_size=32, **kwargs):
        ...
        if self.image_size == 32:
            # change conv1 to accept 3-channel and keep small stride/padding
            self.model.conv1 = nn.Conv2d(3, 64, kernel_size=KERNEL_SIZE,
                                         stride=STRIDE, padding=1, bias=False)
            # remove/disable maxpool if needed (original code used delattr)
            if hasattr(self.model, 'maxpool'):
                # replace with identity to keep attribute
...
class ResNetVAE(nn.Module):
    ...
    def forward(self, x, deterministic=True, classification_only=True):
        # encoder_module expects ResNetEnc which returns flattened vector
        for i_op in self.encoder_module:
            x = i_op(x)
            x = nn.ReLU()(x)
...
# class VAEClassifier(nn.Module):
#     def __init__(self, num_feature_map=64, encoder_layer=3, decoder_layers=4, input_channels=3):
#         super(VAEClassifier, self).__init__()
